...
- **Availability is per EVENT, never per venue** (`src/lib/seats.ts`). A venue's seats are shared by every event held there, so `Seat` carries NO status — occupancy is derived from the `OrderItem`s of that event's live orders (`LIVE_ORDER_STATUSES` = PENDING_PAYMENT | PAYMENT_SUBMITTED | CONFIRMED), the same rule free-capacity zones use. Use `getEventInventory(eventId)` / `getInventoryByEvent(ids)` for reads and `findTakenSeatIds()` inside the order transaction. Never add a global status back onto `Seat`: it made one event's sales block every other event at the same venue (fixed 2026-08-25, migration `seat_availability_per_event`).
- Orders: created in a **serializable transaction** — both seats and free-zone capacity are checked against the live orders *of that event*; Serializable isolation is what makes the read-then-insert safe (a concurrent buyer for the same seat loses with P2034 → 409). 15-min expiry applies only to PENDING_PAYMENT. **No cron**: `expireStaleOrders()` (`src/lib/orders.ts`) runs lazily before order reads/writes; cancelling the order IS the release, there is no inventory row to reset.
...
- Confirmation (organizer): order→CONFIRMED, one ticket per seat/quantity with UUID `code` + QR data URL, buyer notified by email. (No seat rows are touched — a CONFIRMED order is what makes its seats read as SOLD for that event.)
